...
- 📚 **合集系统**：文章可归属合集，读者侧有合集列表页与合集详情页；合集展示顺序与合集内文章章节序均可在后台一键调整（↑↓ 排序，带平滑动画），首页/侧栏/sitemap 按同一顺序输出。
...
- **合集**：创建 / 编辑合集（标题、简介、封面），列表内 ↑↓ 调整合集展示顺序；点合集名进入合集详情，↑↓ 调整文章章节序（从 1 起编号）。两处排序均即时动画、自动保存；未手动排序的合集按创建日期兜底排在后面。
...
- **排序**：合集 tab 的 ↑↓ 调整合集先后（首页合集网格/侧栏组头/sitemap 同序）；合集详情内的 ↑↓ 调整文章章节序（侧栏编号与合集页顺序）。
